...
USER = "GaponovAlexey"
DAYS = 14
MAX_REPOS = 20  # Limit to top 20 most recently updated repos
...
def get_repos():
    ...
    page = 1
    while True:
        data = api(f"https://api.github.com/user/repos?per_page=100&page={page}&affiliation=owner&sort=pushed&direction=desc")
        if not data:
            break
        repos.extend(data)
        if len(data) < 100:
...
def main():
    ...
    print(f"Found {len(repo_names)} repos (limited to {MAX_REPOS} most recently updated)")
