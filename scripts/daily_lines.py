...
def get_repos():
            ...
            page = 1
            while True:
                            data = api(f"https://api.github.com/user/repos?per_page=100&page={page}&affiliation=owner&sort=pushed")
                            if not data:
                                                break
                                            repos.extend(data)
                            if len(data) < 100:
                                                break
                                            page += 1
                        # Sort by pushed_at and take only top 20 most recently updated
                        repos_sorted = sorted(repos, key=lambda r: r.get("pushed_at", ""), reverse=True)
    return repos_sorted[:20]  # Limit to 20 most active repos
...
def main():
    ...
    print(f"Found {len(repo_names)} active repos (limited to top 20 recently updated)")
